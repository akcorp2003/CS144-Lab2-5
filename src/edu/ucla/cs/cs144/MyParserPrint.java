...
class MyParserPrint {
    ...
    static void processFile(File xmlFile, FileWriter writer) {
        ...
        try {
			...
			for(int i = 0; i < itemlist.getLength(); i++){
				...
				for(int j = 0; j < categoryList.getLength(); j++){
					...
					String cat = category.getTextContent();
					writer.append("&&&");
					writer.append(itemID + ',' + cat + '\n');
				...
				if(item.getElementsByTagName("Buy_Price").item(0) != null) {
					buy_price = item.getElementsByTagName("Buy_Price").item(0).getTextContent().substring(1);
				}
				...
				writer.append("%%%"+itemID + ','+name+','+buy_price+','+currently.substring(1)+','+nobids+','+first.substring(1)+','+location+
				...
				for(int x = 0; x < bidlist.getLength(); x++){
					...
					writer.append("###"+itemID+','+bidder_id+','+time+','+amt.substring(1)+'\n');
					
					boolean flag = false;
					for(int y = 0; y < userList.size(); y++){
			    		if(userList.get(y).user_id.equals(bidder_id)){
			    			User new_user = userList.get(y);
				...
				//seller
				for(int y = 0; y < userList.size(); y++){
		    		if(userList.get(y).user_id.equals(userid)){
		    			User new_user = userList.get(y);
				...
				for(User user : userList){
					writer.append("^^^"+user.user_id+','+user.bid_rating+','+user.sell_rating
        ...
        recursiveDescent(itemlist.item(0), 0);
    }
    
    //check if an arraylist of users contains a given userid, if so return the user else return null
    private static User UserListContains(ArrayList<User> userlist, String userid){
